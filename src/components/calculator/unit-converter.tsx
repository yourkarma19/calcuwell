...
export default function UnitConverter() {
  const [conversionType, setConversionType] =
    usePersistentState<ConversionType>("unit-type", "length");

  const [fromUnit, setFromUnit] = useState(
    Object.keys(units[conversionType])[0],
  );
  ...
  const convertedValue = useMemo(() => {
    ...
    if (conversionType === "temperature") {
      ...
      const fromFactor = (units[conversionType] as Record<string, number>)[
        fromUnit
      ];
      const toFactor = (units[conversionType] as Record<string, number>)[
        toUnit
      ];
