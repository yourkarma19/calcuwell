...
export default function LoanComparisonResults({
  resultsA,
  resultsB,
}: LoanComparisonResultsProps) {
  const savings = resultsB.totalAmount - resultsA.totalAmount;
  const verdict =
    savings > 0
      ? { better: "A", amount: savings }
      : { better: "B", amount: Math.abs(savings) };
  ...
  const chartData = [
    ...
    {
      name: "Total Interest",
      "Loan A": resultsA.totalInterest,
      "Loan B": resultsB.totalInterest,
    },
    {
      name: "Total Amount",
      "Loan A": resultsA.totalAmount,
      "Loan B": resultsB.totalAmount,
    },
  ];
      ...
      <Card>
        ...
        <CardContent>
          <div className="bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200 p-4 rounded-lg text-center font-semibold text-lg">
            ✅ Loan Option {verdict.better} is better. You could save{" "}
            {formatter.format(verdict.amount)}.
          </div>
        </CardContent>
      </Card>
      <Card>
            ...
            <table className="w-full text-center">
              ...
              <tbody>
                <tr className="border-b">
                  <td className="p-2 text-left">Monthly EMI</td>
                  <td className="p-2">{formatter.format(resultsA.emi)}</td>
                  <td className="p-2">{formatter.format(resultsB.emi)}</td>
                </tr>
                <tr className="border-b">
                  <td className="p-2 text-left">Total Interest Paid</td>
                  <td className="p-2">
                    {formatter.format(resultsA.totalInterest)}
                  </td>
                  <td className="p-2">
                    {formatter.format(resultsB.totalInterest)}
                  </td>
                </tr>
                <tr>
                  <td className="p-2 text-left font-bold">Total Amount Paid</td>
                  <td className="p-2 font-bold">
                    {formatter.format(resultsA.totalAmount)}
                  </td>
                  <td className="p-2 font-bold">
                    {formatter.format(resultsB.totalAmount)}
                  </td>
                </tr>
              </tbody>
            </table>
