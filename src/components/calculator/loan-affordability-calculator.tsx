...
import AboutLoanAffordabilityCalculator from "./about/loan-affordability-calculator";
...
export default function LoanAffordabilityCalculator({
  ...
  const { affordableLoan, monthlyPayment } = useMemo(() => {
    ...
    const maxMonthlyPaymentForHousing =
      monthlyIncome * (dtiRatio / 100) - monthlyDebt;
    ...
    if (monthlyInterestRate <= 0) {
      return {
        affordableLoan: maxMonthlyPaymentForHousing * numberOfPayments,
        monthlyPayment: maxMonthlyPaymentForHousing,
      };
    }

    const affordableLoanAmount =
      (maxMonthlyPaymentForHousing *
    ...
    return {
      affordableLoan:
        isFinite(affordableLoanAmount) && affordableLoanAmount > 0
          ? affordableLoanAmount
          : 0,
      monthlyPayment:
        maxMonthlyPaymentForHousing > 0 ? maxMonthlyPaymentForHousing : 0,
    };
    ...
    <div className="space-y-6">
      <Card id="loan-affordability-inputs">
        <CardHeader>
          ...
          <CardDescription>
            Enter your financial details to estimate the loan amount you might
            be able to afford.
          </CardDescription>
        </CardHeader>
      ...
      <Card id="loan-affordability-results">
        <CardHeader>
          <CardTitle>Loan Affordability</CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-4" aria-live="polite">
          <div>
            ...
            <p className="text-4xl font-bold font-headline text-primary">
              ₹
              {affordableLoan.toLocaleString("en-IN", {
      ...
      <div className="mt-8">
        <AboutLoanAffordabilityCalculator />
      </div>
      <ExportShareControls
